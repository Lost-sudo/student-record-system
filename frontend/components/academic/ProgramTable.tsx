...
import { AcademicProgramDto, DegreeType } from '@/api/academicPrograms';
...
interface ProgramTableProps {
  ...
  onToggleActive?: (program: AcademicProgramDto) => void;
  onEdit?: (program: AcademicProgramDto) => void;
  onDelete?: (program: AcademicProgramDto) => void;
  isLoading?: boolean;
}
...
const degreeTypeBadge: Record<DegreeType, string> = {
  CERTIFICATE: 'border-sky-500/20 bg-sky-500/10 text-sky-400',
  ASSOCIATE: 'border-teal-500/20 bg-teal-500/10 text-teal-400',
  BACHELOR: 'border-indigo-500/20 bg-indigo-500/10 text-indigo-300',
  MASTER: 'border-violet-500/20 bg-violet-500/10 text-violet-300',
  DOCTORATE: 'border-fuchsia-500/20 bg-fuchsia-500/10 text-fuchsia-300',
};
    ...
    <section className="glass rounded-3xl border border-slate-700/50 shadow-2xl shadow-indigo-900/20 overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-800/40 text-xs text-slate-400 uppercase tracking-wider">
            <tr>
              ...
              <th className="px-6 py-3 text-left font-semibold">Degree Type</th>
          ...
          <tbody className="divide-y divide-slate-700/50">
            {isLoading ? (
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-slate-400">Loading academic programs...</div>
                  </div>
                </td>
              </tr>
            ) : programs.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-slate-400">No academic programs found</div>
              ...
              programs.map((program) => (
                <tr key={program.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="px-6 py-4 max-w-[280px]">
                    ...
                    {program.description && (
                      <div className="text-xs text-slate-400 mt-0.5 truncate">{program.description}</div>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`inline-block px-2.5 py-1 text-xs font-semibold rounded-lg border ${degreeTypeBadge[program.degreeType]}`}
                    >
                      {program.degreeType}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-slate-400">{formatDate(program.createdAt)}</td>
                  <td className="px-6 py-4">
                    <span
      ...
      <div className="px-6 py-4 border-t border-slate-700/50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="text-xs text-slate-400">
          ...
          <span className="text-slate-200 font-semibold">{total.toLocaleString()}</span> programs
        </div>
