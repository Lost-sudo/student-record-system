...
import { AcademicTermDto } from '@/api/academicTerms';
...
interface TermTableProps {
  ...
  onToggleActive?: (term: AcademicTermDto) => void;
  onEdit?: (term: AcademicTermDto) => void;
  onDelete?: (term: AcademicTermDto) => void;
  isLoading?: boolean;
}
    ...
    <section className="glass rounded-3xl border border-slate-700/50 shadow-2xl shadow-indigo-900/20 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-slate-800/40 text-xs text-slate-400 uppercase tracking-wider">
            <tr>
              <th className="px-6 py-3 text-left font-semibold">Term Code</th>
              ...
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-slate-400">Loading academic terms...</div>
                  </div>
                </td>
              </tr>
            ) : terms.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-slate-400">No academic terms found</div>
              ...
              terms.map((term) => (
                <tr key={term.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="px-6 py-4">
                    <span className="inline-block px-2.5 py-1 text-xs font-mono font-semibold rounded-lg border border-emerald-500/20 bg-emerald-500/10 text-emerald-400">
                      {term.termCode}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-semibold text-white">{term.name}</div>
                    <div className="text-xs text-slate-500">
                      Created {formatDate(term.createdAt)}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-slate-400">{formatDate(term.startDate)}</td>
                  <td className="px-6 py-4 text-slate-400">{formatDate(term.endDate)}</td>
                  <td className="px-6 py-4">
                    <span
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        ...
                        aria-label={term.isActive ? 'Deactivate term' : 'Activate term'}
      ...
      <div className="px-6 py-4 border-t border-slate-700/50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="text-xs text-slate-400">
          ...
          <span className="text-slate-200 font-semibold">{total.toLocaleString()}</span> terms
        </div>
