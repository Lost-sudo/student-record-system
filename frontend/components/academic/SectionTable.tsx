...
import { CourseSectionDto } from '@/api/courseSections';
...
interface SectionTableProps {
  ...
  onEdit?: (section: CourseSectionDto) => void;
  onDelete?: (section: CourseSectionDto) => void;
  isLoading?: boolean;
}
...
const getEnrollmentTone = (enrolledCount: number, capacity: number) => {
  const ratio = capacity > 0 ? enrolledCount / capacity : 1;
  if (ratio >= 1) return { bar: 'bg-red-500', text: 'text-red-400' };
  if (ratio >= 0.7) return { bar: 'bg-amber-500', text: 'text-amber-400' };
    ...
    <section className="glass rounded-3xl border border-slate-700/50 shadow-2xl shadow-indigo-900/20 overflow-hidden">
              ...
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-slate-400">Loading course sections...</div>
                  </div>
                </td>
              </tr>
            ) : sections.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-slate-400">No course sections found</div>
                    <div className="text-xs text-slate-500">Try adjusting your filters</div>
                  </div>
                </td>
              </tr>
            ) : (
              sections.map(({ section, courseCode, courseTitle, termLabel }) => {
                const tone = getEnrollmentTone(section.enrolledCount, section.capacity);
                const fillPct = Math.min(100, Math.round((section.enrolledCount / Math.max(1, section.capacity)) * 100));
                ...
                return (
                  <tr key={section.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="px-6 py-4 max-w-[240px]">
                      ...
                      <div className="text-sm text-slate-300 mt-1 truncate">{courseTitle}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center justify-center min-w-[36px] px-2.5 py-1 text-xs font-bold rounded-lg border border-indigo-500/20 bg-indigo-500/10 text-indigo-300">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2.5">
                        <div className="w-16 h-1.5 rounded-full bg-slate-700 overflow-hidden shrink-0">
                          <div
                    ...
                    <td className="px-6 py-4 text-slate-400">{formatDate(section.createdAt)}</td>
      ...
      <div className="px-6 py-4 border-t border-slate-700/50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="text-xs text-slate-400">
          ...
          <span className="text-slate-200 font-semibold">{total.toLocaleString()}</span> sections
        </div>
