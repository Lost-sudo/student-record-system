...
import { StudentCreationError } from '@/api/students';

interface StudentWizardFormProps {
  defaultFormData?: Partial<AddStudentFormData>;
  submitLabel?: string;
...
export const STEP_PREFIXES = ['studentInfo', 'contactInfo', 'emergencyContact'] as const;

export default function StudentWizardForm({
  defaultFormData,
  ...
  const applyServerErrors = (error: StudentCreationError) => {
    setErrorSteps((prev) =>
  ...
  const onFinalSubmit = async () => {
    ...
    try {
      ...
      if (error instanceof StudentCreationError) {
        ...
        sonnerToast.error('Unable to save student', {
          description: error.message,
        });
      } else {
        sonnerToast.error('Failed to save student', {
